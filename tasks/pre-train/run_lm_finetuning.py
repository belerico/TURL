...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            model.train()
            outputs = model(
                input_ids=input_tok, attention_mask=input_mask, token_type_ids=input_type, masked_lm_labels=labels
            )
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    eval_dataset = WikiTableDataset(
        args.data_dir, src="dev", max_length=[50, 10, 10], force_new=False, tokenizer=tokenizer
    )
    ...
    eval_dataloader = TableLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size)
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset = WikiTableDataset(
            args.data_dir, src="train", max_length=[50, 10, 10], force_new=False, tokenizer=tokenizer
        )
