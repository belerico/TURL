...
class TFAdaptiveSoftmaxMask(tf.keras.layers.Layer):
    def __init__(self, n_token, d_embed, d_proj, cutoffs, div_val=1, keep_order=False, **kwargs):
        ...
        if self.div_val == 1:
            for i in range(len(self.cutoffs)):
                ...
                bias = self.add_weight(
                    shape=(self.n_token,), initializer="zeros", trainable=True, name="out_layers_._{}_._bias".format(i)
                )
            ...
            for i in range(len(self.cutoffs)):
                ...
                bias = self.add_weight(
                    shape=(r_idx - l_idx,), initializer="zeros", trainable=True, name="out_layers_._{}_._bias".format(i)
                )
                self.out_layers.append((weight, bias))
