...
def tokenize_table(table, config):
    ...
    tokenized_cells = [
        [index, config.tokenizer.encode(cell, max_length=config.max_cell_length, add_special_tokens=False)]
    ...
    len(input_tok) + len(input_ent_text)
...
def tokenize_table_for_bert(table, config):
    ...
    len(tokenized_meta)
    input_tok += tokenized_meta
    [len(z) for z in tokenized_headers]
    input_tok += list(itertools.chain(*tokenized_headers))
...
def process_single_TR(input_data, config):
    ...
    for table in pos:
        ...
        input_tok_ent_mask = np.concatenate(
            [np.ones([tokenized_query_length + 1, len(input_ent_text)], dtype=int), input_tok_ent_mask], axis=0
        )
        input_ent_mask = (
            np.concatenate(
                [np.ones([len(input_ent_text), tokenized_query_length + 1], dtype=int), input_ent_mask[0]], axis=1
            ),
            input_ent_mask[1],
        )
        pos_inputs.append(
...
class WebQueryTableDataset(Dataset):
    def _preprocess(self, data_dir):
        if not self.for_bert:
            preprocessed_filename = os.path.join(data_dir, "procressed_TR", self.src)
        else:
            preprocessed_filename = os.path.join(data_dir, "procressed_TR_Bert", self.src)
            ...
            with open(os.path.join(data_dir, "WQT.dataset.query.tsv"), "r", encoding="utf-8") as f:
                ...
                for i, q in enumerate(tqdm(f)):
                    ...
                    if q[2] == "train":
                        train_q_t[i] = [
                            i,
                            self.tokenizer.encode(q[1], max_length=self.max_query_length, add_special_tokens=False),
                            [],
                            [],
                        ]
                    elif q[2] == "dev":
                ...
                for q, t in enumerate(tqdm(f)):
                    ...
                    for i, row in enumerate(rows):
                        ...
                        if j + 1 != column_num:
                            ...
                            cells = []
                            break
                    tables.append([t[2] + " " + t[3], headers, cells])
                    assert q == int(t[0])
                pool = Pool(processes=5)
                tokenized_tables = list(
                    tqdm(
                        pool.imap(
                            partial(tokenize_table if not self.for_bert else tokenize_table_for_bert, config=self),
                            tables,
                            chunksize=1000,
                        ),
                        total=len(tables),
        ...
        processed_train_q_t = list(
            tqdm(
                pool.imap(
                    partial(process_single_TR if not self.for_bert else process_single_TR_forBert, config=self),
                    list(train_q_t.items()),
                    chunksize=1000,
                ),
                total=len(train_q_t),
            )
        )
        processed_dev_q_t = list(
            tqdm(
                pool.imap(
                    partial(process_single_TR if not self.for_bert else process_single_TR_forBert, config=self),
                    list(dev_q_t.items()),
        ...
        with open(
            os.path.join(data_dir, "procressed_TR" if not self.for_bert else "procressed_TR_Bert", "train.pickle"), "wb"
        ) as f:
            pickle.dump(processed_train_q_t, f)
        with open(
            os.path.join(data_dir, "procressed_TR" if not self.for_bert else "procressed_TR_Bert", "dev.pickle"), "wb"
        ) as f:
            pickle.dump(processed_dev_q_t, f)
        with open(
            os.path.join(data_dir, "procressed_TR" if not self.for_bert else "procressed_TR_Bert", "test.pickle"), "wb"
        ) as f:
...
class finetune_collate_fn_TR:
    ...
    def __call__(self, raw_batch):
        ...
        if self.is_train:
            batch_neg_inputs = [
                random.sample(x, self.neg_num) if len(x) > self.neg_num else x for x in batch_neg_inputs
            ]
...
class TRLoader(DataLoader):
    ...
    def __init__(
        self, dataset, batch_size, shuffle=True, is_train=True, num_workers=0, sampler=None, neg_num=5, for_bert=False
    ):
