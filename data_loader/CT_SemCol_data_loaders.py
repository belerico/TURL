...
def process_single_CT(input_data, config):
    ...
    for e_i, (index, entity) in enumerate(entities):
        tokenized_ent_text = config.tokenizer.encode(
            entities_text[e_i], max_length=config.max_cell_length, add_special_tokens=False
        )
...
class SemColDataset(Dataset):
    def _preprocess(self, data_dir):
        preprocessed_filename = os.path.join(data_dir, "procressed_CT", self.src)
                            ...
                            for j in range(col_num - 1):
                                for i, cell in enumerate(m_table["col_N_%d" % j]):
                                    if cell != "" and cell != "NaN":
                                        ...
                                        entities_text.append(cell)
                            tables.append([col_id, entities, entities_text, class_vocab[cls]])
                return tables
        ...
        print(
            "{} train tables, {} test t2d tables, {} test limaye tables, {} test wikipedia tables".format(
                len(train_tables), len(test_tables_t2d), len(test_tables_limaye), len(test_tables_wiki)
            )
        )
        pool = Pool(processes=4)
        processed_train_tables = list(
            tqdm(
                pool.imap(partial(process_single_CT, config=self), train_tables, chunksize=1000),
                total=len(train_tables),
            )
        )
        processed_test_tables_t2d = list(
            tqdm(
                pool.imap(partial(process_single_CT, config=self), test_tables_t2d, chunksize=1000),
                total=len(test_tables_t2d),
            )
        )
        processed_test_tables_limaye = list(
            tqdm(
                pool.imap(partial(process_single_CT, config=self), test_tables_limaye, chunksize=1000),
                total=len(test_tables_limaye),
            )
        )
        processed_test_tables_wiki = list(
            tqdm(
                pool.imap(partial(process_single_CT, config=self), test_tables_wiki, chunksize=1000),
                total=len(test_tables_wiki),
            )
        )
        pool.close()
        # pdb.set_trace()
