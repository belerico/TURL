...
def process_single_hybrid_table(
    ...
    if subject in header_span:
        ...
        new_input_ent_mask[0][
            1, tokenized_meta_length + header_span[subject][0] : tokenized_meta_length + header_span[subject][1]
        ] = 1
    ...
    if src == "train":
        ...
        )  # mask entity in the same column for prediction
        for e_i, (index, _) in enumerate(entities):
            tmp_j = 0
...
class WikiHybridTableDataset(Dataset):
    def _preprocess(self, data_dir):
        ...
        if self.mode == 0:
            ...
            processed_data = list(
                tqdm(
                    pool.imap(partial(process_single_hybrid_table_CER, config=self), actual_tables, chunksize=2000),
                    total=len(actual_tables),
                )
            )
        # elif self.mode == 2:
...
class pretrain_hybrid_table_collate_fn:
    ...
    def __call__(self, raw_batch):
        ...
        batch_input_ent_final, batch_input_ent_mask_type, batch_input_ent_labels = mask_ent(
            ...
            mall_probability=self.mall_probability,
...
class HybridTableLoader(DataLoader):
    ...
    def __init__(
        ...
        if self.mode == 0:
            ...
            self.collate_fn = pretrain_hybrid_table_collate_fn_CER(
                dataset.tokenizer,
                dataset.entity_wikid2id,
                self.max_entity_candidate,
                is_train=self.is_train,
