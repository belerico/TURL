...
def process_single_EL(input_data, config):
    ...
    for name, description, types in candidate_entities:
        ...
        cand_description.append(
            config.tokenizer.encode(description, max_length=config.max_description_length, add_special_tokens=False)
        )
    ...
    tokenized_headers = [
        config.tokenizer.encode(z, max_length=config.max_header_length, add_special_tokens=False) for z in headers
    ]
    input_tok = []
    input_tok_pos = []
    input_tok_type = []
    ...
    new_input_ent_mask = [
        np.ones([len(input_ent_text), len(input_tok)], dtype=int),
        np.ones([len(input_ent_text), len(input_ent_text)], dtype=int),
    ]
    ...
    return [
        ...
        cand_name_padded,
        cand_name_length,
        cand_description_padded,
        cand_description_length,
        cand_type_padded,
        cand_type_length,
        len(cand_name_length),
        np.array(labels),
        entities_index,
    ]
...
class ELDataset(Dataset):
    def _preprocess(self, data_dir):
        preprocessed_filename = os.path.join(data_dir, "procressed_EL", self.src)
...
class finetune_collate_fn_EL:
    ...
    def __call__(self, raw_batch):
        (
            ...
            batch_cand_name,
            batch_cand_name_length_tmp,
            batch_cand_description,
            batch_cand_description_length_tmp,
            batch_cand_type,
            batch_cand_type_length_tmp,
            batch_cand_length,
            batch_labels,
            batch_entities_index,
        ) = zip(*raw_batch)
        ...
        # pdb.set_trace()
        for i, (tok_l, ent_l, cand_l) in enumerate(
            zip(batch_input_tok_length, batch_input_ent_length, batch_cand_length)
        ):
        ...
        return (
            ...
            batch_input_ent_mask_padded,
            batch_cand_name_padded,
            batch_cand_name_length,
            batch_cand_description_padded,
            batch_cand_description_length,
            batch_cand_type_padded,
            batch_cand_type_length,
            batch_cand_mask_padded,
            batch_labels_padded,
            batch_entities_index,
        )
