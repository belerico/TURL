...
def process_single_header(input_data, config):
    ...
    return [
        ...
        np.array(input_headers),
        header_mask,
        len(tokenized_headers),
    ]


class WikiHeaderDataset(Dataset):
    def _preprocess(self, data_dir):
        preprocessed_filename = os.path.join(data_dir, "procressed_HR", self.src)
        ...
        processed_table_headers = list(
            tqdm(
                pool.imap(partial(process_single_header, config=self), table_headers, chunksize=1000),
                total=len(table_headers),
            )
        )
        pool.close()
        # pdb.set_trace()
...
class finetune_collate_fn_Header:
    ...
    def __call__(self, raw_batch):
        (
            ...
            batch_tokenized_meta_length,
            batch_input_header,
            batch_header_mask,
            batch_input_header_length,
        ) = zip(*raw_batch)

        max_input_tok_length = max(batch_input_tok_length)
        max(batch_input_header_length)
        ...
        for i, (tok_l, header_l, meta_l) in enumerate(
            zip(batch_input_tok_length, batch_input_header_length, batch_tokenized_meta_length)
        ):
...
class WikiHeaderLoader(DataLoader):
    ...
    def __init__(self, dataset, batch_size, shuffle=True, is_train=True, num_workers=0, sampler=None, seed=1):
        ...
        self.is_train = is_train
        self.collate_fn = finetune_collate_fn_Header(
            dataset.tokenizer, len(dataset.header_vocab), seed=seed, is_train=self.is_train
        )
