...
def process_single_RE(input_data, config):
    ...
    column_entity_mask = np.concatenate(
        [np.zeros([len(relation_annotations) + 1, 1], dtype=int), column_entity_mask], axis=1
    )
...
class REDataset(Dataset):
    def _preprocess(self, data_dir):
        preprocessed_filename = os.path.join(data_dir, "procressed_RE", self.src)
        ...
        processed_cols = list(
            tqdm(pool.imap(partial(process_single_RE, config=self), cols, chunksize=1000), total=len(cols))
        )
        pool.close()
        # pdb.set_trace()
...
def process_single_REBERT(input_data, config):
    ...
    for i, tokenized_header in enumerate(tokenized_headers[1:]):
        ...
        input_tok += tokenized_meta
        input_tok += (
            [config.tokenizer.convert_tokens_to_ids("[unused0]")]
            + tokenized_headers[0]
            + [config.tokenizer.convert_tokens_to_ids("[unused1]")]
        )
        input_tok += (
            [config.tokenizer.convert_tokens_to_ids("[unused2]")]
            + tokenized_header
            + [config.tokenizer.convert_tokens_to_ids("[unused3]")]
        )
...
class REBERTDataset(Dataset):
    def _preprocess(self, data_dir):
        preprocessed_filename = os.path.join(data_dir, "procressed_REBERT", self.src)
        preprocessed_filename += ".pickle"
        if not self.force_new and os.path.exists(preprocessed_filename):
            print("try loading preprocessed data from %s" % preprocessed_filename)
            with open(preprocessed_filename, "rb") as f:
        ...
        pool = Pool(processes=4)
        processed_cols = list(
            tqdm(pool.imap(partial(process_single_REBERT, config=self), cols, chunksize=1000), total=len(cols))
        )
