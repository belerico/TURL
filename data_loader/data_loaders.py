...
def process_single_table(input_table, config):
    ...
    tokenized_meta = (
        config.tokenizer.encode(pgTitle, max_length=config.max_title_length)
    ...
    assert input_length <= config.max_tokenized_input, "max set as %d, run into input with length %d" % (
        config.max_tokenized_input,
        input_length,
    )
    ...
    for i_0 in range(row_num):
        ...
        for i_1 in range(row_num):
            if i_0 == i_1:
                cell_cell_mask[i_0].append(
                    np.ones([sum(tokenized_rows_length[i_0]), sum(tokenized_rows_length[i_1])], dtype=int)
                )
            elif i_1 > i_0:
                ...
                for j in range(column_num):
                    tmp_mask[
                        last_i : last_i + tokenized_rows_length[i_0][j], last_j : last_j + tokenized_rows_length[i_1][j]
                    ] = 1
...
class WikiTableDataset(Dataset):
    def _preprocess(self, data_dir):
        preprocessed_filename = os.path.join(data_dir, "procressed", self.src)
        ...
        processed_data = list(
            tqdm(
                pool.imap(partial(process_single_table, config=self), actual_tables, chunksize=1000),
...
class pretrain_table_collate_fn:
    ...
    def __call__(self, raw_batch):
        ...
        batch_input_tok_padded = torch.nn.utils.rnn.pad_sequence(
            batch_input_tok, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )
        ...
        batch_input_tok_final, labels = mask_tokens(
            batch_input_tok_padded, self.tokenizer, mlm_probability=self.mlm_probability
        )
...
class TableLoader(DataLoader):
    ...
    def __init__(self, dataset, batch_size, shuffle=True, num_workers=0, mlm_probability=0.15, sampler=None):
        self.mlm_probability = mlm_probability
...
def process_single_entity_table(input_table, config):
    ...
    if 0 in header_span:
        ...
        new_input_ent_mask[0][
            1, tokenized_meta_length + header_span[0][0] : tokenized_meta_length + header_span[0][1]
        ] = 1
    ...
    if config.src == "train":
        ...
        exclusive_ent_mask = np.full(
            [len(input_ent), max([len(z) for _, z in column_en_map.items()]) - 1], 1000
        )  # mask entity in the same row for prediction
        for e_i, (index, _) in enumerate(entities):
            tmp_j = 0
    ...
    return [
        ...
        len(input_tok),
        np.array(input_ent),
        np.array(input_ent_local_id),
        np.array(input_ent_type),
        np.array(input_ent_pos),
        (np.array(input_ent_mask[0]), np.array(input_ent_mask[1])),
        len(input_ent),
        np.array(core_entity_mask),
        core_ent_local_id,
        all_entity_set,
        entity_cand,
        exclusive_ent_mask,
...
def process_single_entity_table_CER(input_table, config):
    ...
    if caption != secTitle:
        tokenized_meta += config.tokenizer.encode(caption, max_length=config.max_title_length, add_special_tokens=False)
    tokenized_header = config.tokenizer.encode(
        headers[0][1], max_length=config.max_header_length, add_special_tokens=False
    )
    input_tok = []
    input_tok_pos = []
    input_tok_type = []
...
class WikiEntityTableDataset(Dataset):
    def _preprocess(self, data_dir):
        if self.mode == 0:
            ...
            preprocessed_filename = os.path.join(data_dir, "procressed_CER", self.src)
        else:
            raise Exception
        preprocessed_filename += ".pickle"
        if not self.force_new and os.path.exists(preprocessed_filename):
        ...
        actual_table_num = len(actual_tables)
        print(
            "%d original tables, actual %d tables in total\n%d tables removed because of extra entity filtering"
        ...
        if self.mode == 0:
            processed_data = list(
                tqdm(
                    pool.imap(partial(process_single_entity_table, config=self), actual_tables, chunksize=1000),
                    total=len(actual_tables),
                )
            )
        elif self.mode == 1:
            processed_data = list(
                tqdm(
                    pool.imap(partial(process_single_entity_table_CER, config=self), actual_tables, chunksize=1000),
                    total=len(actual_tables),
                )
            )
        # elif self.mode == 2:
...
def mask_ent(inputs_origin, inputs_local_id, core_entity_mask, entity_wikid2id, mlm_probability=0.15, is_train=False):
    ...
    if is_train:
        # We sample a few tokens in each sequence for masked-LM training (with probability args.mlm_probability defaults to 0.15 in Bert/RoBERTa)
        probability_matrix = torch.full(labels.shape, mlm_probability)
        special_tokens_mask = inputs < len(
            RESERVED_ENT_VOCAB
        )  # [list(map(lambda x: 1 if x == entity_wikid2id['[PAD]'] else 0, val)) for val in labels.tolist()]
        ...
        random_words = torch.randint(
            low=RESERVED_ENT_VOCAB_NUM, high=len(entity_wikid2id), size=labels.shape, dtype=torch.long
        )
        inputs[indices_random] = random_words[indices_random]
...
class pretrain_entity_table_collate_fn:
    ...
    def generate_random_candidate_v2(self, batch_size, masked_entity, candidate_distribution=None, ent_cand=None):
        ...
        final_candidates = np.tile(
            np.argpartition(random_shifts, self.max_entity_candidate)[: self.max_entity_candidate], [batch_size, 1]
        )
    ...
    def __call__(self, raw_batch):
        ...
        (
            ...
            batch_input_ent,
            batch_input_ent_local_id,
            batch_input_ent_type,
            batch_input_ent_pos,
            batch_input_ent_mask,
            batch_input_ent_length,
            batch_core_entity_mask,
            batch_core_ent_local_id,
            batch_all_entity_set,
            batch_entity_cand,
            batch_exclusive_ent_mask,
        ) = zip(*raw_batch)
        ...
        if self.is_train:
            ...
            batch_exclusive_ent_mask_padded = np.full(
                [batch_size, max_input_ent_length, max_input_col_ent_num], 1000, dtype=int
            )
        ...
        for i, (tok_l, ent_l) in enumerate(zip(batch_input_tok_length, batch_input_ent_length)):
            ...
            if self.is_train:
                batch_exclusive_ent_mask_padded[
                    i, :ent_l, : batch_exclusive_ent_mask[i].shape[1]
                ] = batch_exclusive_ent_mask[i]
        ...
        batch_input_tok_final, batch_input_tok_labels = mask_tokens(
            batch_input_tok_padded, self.tokenizer, mlm_probability=self.mlm_probability
        )
        batch_input_ent_final, batch_input_ent_labels = mask_ent(
            batch_input_ent_padded,
            batch_input_ent_local_id_padded,
            batch_core_entity_mask_padded,
            self.entity_wikid2id,
            mlm_probability=self.ent_mlm_probability,
        ...
        batch_candidate_entity_set = self.generate_random_candidate_v2(
            batch_size, batch_all_entity_set, self.candidate_distribution, ent_cand=batch_entity_cand
        )
        ...
        return (
            batch_table_id,
            batch_input_tok_final,
            batch_input_tok_type_padded,
            batch_input_tok_pos_padded,
            batch_input_tok_labels,
            batch_input_tok_mask_padded,
            batch_input_ent_final,
            batch_input_ent_type_padded,
            batch_input_ent_pos_padded,
            batch_input_ent_labels,
            batch_input_ent_mask_padded,
            batch_candidate_entity_set,
            batch_core_entity_label,
            batch_exclusive_ent_mask_padded,
            batch_core_entity_mask_padded,
        )
...
class pretrain_entity_table_collate_fn_CER(pretrain_entity_table_collate_fn):
    ...
    def __call__(self, raw_batch):
        ...
        # random sample candidate
        batch_candidate_entity_set = self.generate_random_candidate_v2(
            batch_size, batch_core_entities, self.candidate_distribution, ent_cand=batch_entity_cand
        )
        batch_candidate_entity_set = torch.LongTensor(batch_candidate_entity_set)
        return (
            batch_table_id,
...
def CER_build_input(pgEnt, pgTitle, secTitle, caption, header, seed_entities, entity_cand, config):
    ...
    input_ent = [
        config.entity_wikid2id[pgEnt] if pgEnt != -1 else 0,
        config.entity_wikid2id["[CORE_ENT_MASK]"],
        config.entity_wikid2id["[CORE_ENT_MASK]"],
    ] + input_ent
...
class EntityTableLoader(DataLoader):
    ...
    def __init__(
        ...
        self.mode = mode
        if self.mode == 0:
            self.collate_fn = pretrain_entity_table_collate_fn(
                dataset.tokenizer,
                dataset.entity_wikid2id,
                self.mlm_probability,
                self.ent_mlm_probability,
                self.max_entity_candidate,
                is_train=self.is_train,
                candidate_distribution=self.sample_distribution,
                use_cand=self.use_cand,
            )
        elif self.mode == 1:
            self.collate_fn = pretrain_entity_table_collate_fn_CER(
                dataset.tokenizer,
                dataset.entity_wikid2id,
                self.mlm_probability,
                self.ent_mlm_probability,
