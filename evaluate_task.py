...
def evaluate_CER(args, dataset, seed_num=1, debug=False):
    ...
    with open(os.path.join(args.data_dir, "dev_tables.jsonl"), "r") as f, open(
        args.result_file, "w", encoding="utf-8"
    ) as f_out:
        for line in tqdm(f):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", default=None, type=str, required=True, help="The checkpoint for trained model.")
    parser.add_argument("--result_file", default=None, type=str, required=False, help="The directory to store result.")
    parser.add_argument("--cached_baseline", default=None, type=str, required=False, help="Baseline results")
    parser.add_argument("--task", default=None, type=str, required=True, help="Task to evaluate")
